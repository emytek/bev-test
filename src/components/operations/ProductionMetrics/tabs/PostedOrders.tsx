...
// import { FiSearch, FiFilter, FiPlusCircle, FiDownload, FiEye, FiEdit2, FiPrinter, FiList } from 'react-icons/fi';
...
//   const [postedOrders, setPostedOrders] = useState<ProductionOrderHeader[]>([]); // Changed state name
...
//   const { user } = useAuth(); // Assuming useAuth provides user data if needed for API calls
//   const navigate = useNavigate();

//   // userData might be needed for the `get-approved-production` quick view if it requires it
//   // const userData = user?.id;

//   // --- Data Fetching for All Posted Orders (Table) ---
//   const fetchPostedProductionOrders = useCallback(async () => { // Renamed function
//     setLoading(true);
//     setError(null);
//     try {
//       const response = await axiosInstance.get<{ data: ProductionOrderHeader[]; message: string; status: boolean }>(
//         GET_ALL_POSTED_PRODUCTIONS_ENDPOINT // NEW API ENDPOINT
//       );
//       if (response.data.status) {
//         setPostedOrders(response.data.data); // Set to postedOrders state
//       } else {
...
//       setLoading(false);
//     }
//   }, []); // No external dependencies for fetch function

//   useEffect(() => {
//     fetchPostedProductionOrders();
//   }, [fetchPostedProductionOrders]);

//   // --- Filtering Logic (retained, but applies to postedOrders) ---
//   const filteredOrders = useMemo(() => {
//     return postedOrders.filter(order => // Filter postedOrders
...
//     // Navigate to the ApprovedProductionOrderDetailsPage
//     // This page handles both approved and posted states, fetching by SAPProductionOrderID
//     navigate(`/approved-production-order-details/${sapProductionOrderID}`);
//   }, [navigate]);
...
//     try {
//       // Reuse GET_SINGLE_APPROVED_PRODUCTION_ENDPOINT assuming it can fetch details for posted orders
//       const response = await axiosInstance.get<{ data: ProductionOrderHeader; message: string; status: boolean }>(
...
//           <h2 className="text-2xl font-semibold text-gray-700 dark:text-white">Posted Production Orders</h2> {/* Updated Title */}
...
//             No posted production orders found. {/* Updated empty state message */}
//           </div>
//         )}

//         {/* Pagination (Conceptual - You had this in previous code, adjust if you want to implement it fully here) */}
...
//             <Button onClick={() => toast.info('Previous page functionality coming soon!')} variant="outline" disabled>Previous</Button>
//             <Button onClick={() => toast.info('Next page functionality coming soon!')} variant="outline" disabled>Next</Button>
//           </div>
//         </div>
...
//                 <p><strong>Is Posted:</strong> {selectedOrderForModal.IsPosted ? 'Yes' : 'No'}</p> {/* Highlight IsPosted */}
//               </div>
...
const PostedOrdersManagementTab: React.FC = () => {
  ...
  const sortedAndFilteredOrders = useMemo(() => {
    let currentOrders = postedOrders.filter(order =>
      order.SAPProductDescription.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.SAPProductionOrderID.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.SAPProductID.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  }, [searchTerm, postedOrders, sortColumn, sortDirection]);

  // --- Pagination Logic (now uses sortedAndFilteredOrders) ---
